...
{	
	...
	{
		OS_Lines_Draw(x1 + 1,y1,x2,y1,color);
		OS_Lines_Draw(x1,y2,x2 - 1,y2,color);
		OS_Lines_Draw(x2,y1 + 1,x2,y2,color);
		OS_Lines_Draw(x1,y1,x1,y2 - 1,color);
	} 
	TextColor=temp;
} 
...
{   
	...
	width = strlen((char *)( Struct_Button ->ch));  
	width *=  Struct_Button ->Size/2; 
	
	x1 = Struct_Button ->x1;
	y1 = Struct_Button ->y1;
	x2 = Struct_Button ->x1 + width; 
	y2 = y1 + Struct_Button ->Size; 
	Struct_Button ->x2 = x2 + Struct_Button ->Size;
	Struct_Button ->y2 = y2+Struct_Button ->Size/2;

	
	
	if(Touch_Judge(Struct_Button ->x1 - 3,Struct_Button ->y1 - 3,Struct_Button ->x2 + 5,Struct_Button ->y2 +5) == TOUCH_VALID)
	{
		Struct_Button ->State = 1;
		Struct_Button ->LastState = 1;

	}
	else 
	{
		Struct_Button ->State = 0;
		if(Struct_Button ->LastState == 1) Struct_Button ->LastState = 2;
	} 
	
	if(Mode == 1)
	{
			
		if(Struct_Button ->State == 0)
		{
			OS_Rect_Draw(x1, y1, x2 + Struct_Button ->Size, y2+Struct_Button ->Size/2,0, Struct_Button ->BackColor);  
			OS_Rect_Draw( x1 + 1, y1 + 1, x2 + Struct_Button ->Size - 1, y2+Struct_Button ->Size/2 - 1,0, Struct_Button ->BackColor); 
		}
		else if(Struct_Button ->State == 1)//被按下
		{
			OS_Rect_Draw(x1, y1, x2 + Struct_Button ->Size, y2+Struct_Button ->Size/2,0, ~Struct_Button ->BackColor);  
			OS_Rect_Draw( x1 + 1, y1 + 1, x2 + Struct_Button ->Size - 1, y2+Struct_Button ->Size/2 - 1,0, ~Struct_Button ->BackColor); 
		} 
		 
		TextColor =  Struct_Button ->TextColor;
		BackColor = BLACK;  	
		OS_String_Show( x1 + Struct_Button ->Size/2,y1 + Struct_Button ->Size/4,Struct_Button ->ch,Struct_Button ->Size,1);  	
			
	}
	else
	{
		if(Struct_Button ->Last !=  Struct_Button ->State)
		{
			Struct_Button ->Last = Struct_Button ->State;
			if(Struct_Button ->State == 0)
			{
				OS_Rect_Draw(x1, y1, x2 + Struct_Button ->Size, y2+Struct_Button ->Size/2,0, Struct_Button ->BackColor);  
				OS_Rect_Draw( x1 + 1, y1 + 1, x2 + Struct_Button ->Size - 1, y2+Struct_Button ->Size/2 - 1,0, Struct_Button ->BackColor); 
