...
Struct_Message Message_USART2 = {0};		  
...
/**----------------------------------------------------------------------------
* @FunctionName  : Message_Handle_USART2()
* @Description   : 通信接收
* @Data          : 2016/11/18
* @Explain       : 放置在串口接收中断中
------------------------------------------------------------------------------*/
void Message_Handle_USART2(void)
{
    static u32 time = 0, Clock = 0;
    static u16 Receive_Cnt = 0;

    if(TIM_GetCounter(TIM2) + (OS_Clock - Clock) * 0xffff - time > MESSAGE_GAP)
    {
        Receive_Cnt = 0;
        Message_USART2.Receive_Flag = 0;//开始接收 
    }

    Message_USART2.Receive_Buff[Receive_Cnt ++] = USART_ReceiveData(USART2);//读取接收到的数据

    if(Receive_Cnt >= MESSAGE_MASTER_LENTH)
    {
        if(Message_USART2.Receive_Buff[0] == 0xAA)
            if(Message_USART2.Receive_Buff[MESSAGE_MASTER_LENTH - 1] == 0x55)
                if(Message_CRC16_Receive(Message_USART2.Receive_Buff)) //CRC16校验
                    if(Message_USART2.Receive_Buff[MESSAGE_MASTER_BYTE.ADDRESS_TARGET] == MESSAGE_ADDRESS_LOCAL   //判断目标地址是否为自己
                            || Message_USART2.Receive_Buff[MESSAGE_MASTER_BYTE.ADDRESS_TARGET] == MESSAGE_ADDRESS_RADIO)	//判断目标地址是否为广播 
						{
							Message_USART2.Receive_Flag = 1;//接收完毕 
							Message_Analysis(); 
							LED_PIN(2) = !LED_PIN(2);  
						}
...
* @Explain       : None
------------------------------------------------------------------------------*/
void Message_Send_USART2(void)
{
    int i = 0;
    Message_USART2.Send_Buff[0] = 0xAA;//起始符
    Message_USART2.Send_Buff[MESSAGE_LENTH - 1] = 0x55;//结束符
    Message_USART2.Send_Buff[MESSAGE_BYTE.ADDRESS_LOCAL] = MESSAGE_ADDRESS_LOCAL;//本机地址
	Message_USART2.Send_Buff[MESSAGE_BYTE.ADDRESS_TARGET] = 0x01;//主机
	
    Message_CRC16_Send(Message_USART2.Send_Buff);
	
    for(i = 0; i < MESSAGE_LENTH; i ++)
    {
		while(USART_GetFlagStatus(USART2, USART_FLAG_TC) != SET){}; //等待发送结束 	
        USART_SendData(USART2, (u16) Message_USART2.Send_Buff[i]); //向上位机发送数据	
